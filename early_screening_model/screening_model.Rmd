...
### check why the stages have the unexpected scores, especially the stage IV
### use the 200 features for the final model
...
### use the lung_clinic_used$Nmeta && lung_clinic_used$Nnodes to build model and classify them
```{r}
### 96 samples selected
...
```{r}
### try
training_range_sample$sample <- rownames(training_range_sample)
training_range_sample <- left_join(training_range_sample,lung_clinic_used,by='sample')

training_zscore_rangesamples$sample <- rownames(training_zscore_rangesamples)
training_zscore_rangesamples <- left_join(training_zscore_rangesamples,lung_clinic_used,by='sample')


table(training_range_sample$stage_less)

```
### try rank values sum (use clinic_rank && train_topbins)
```{r}
...
### only the up or down sort of (healthy,I,III,IV)
...
```
### use the 200 fetures to make the model
```{r}
### build the model based on features:rank_sites
  ...
  CAnonCAp=tss_z_pCAcut #19650
   ...
   modeltss_CA = modeltss_CA200  # 
   training_zscore = CAtrain
  testing_zscore = CAtest 
  train_topbins<-as.data.frame(training_zscore)[,rank_sites]
  dim(train_topbins) #  158 300
  test_topbins<-as.data.frame(testing_zscore)[,rank_sites]
  dim(test_topbins) # 52 300
  ...
  pred <- predict(modeltss_CA200,newdata = test_topbins,type = "prob")
